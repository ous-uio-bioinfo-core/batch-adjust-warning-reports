...
```{r runparameters, results='hide', echo=TRUE}
starttime = Sys.time()
debug = FALSE
downloaddata=TRUE
set.seed(100)
...
The data used by Towfic et al. deposited in GEO with accession GSE61901. We need the sample annotation and non-normalized data.
...
# round(sort( sapply(mget(ls()),object.size) )/2^20, 0)
...
For the reproduction, we initially used the raw data from the GEO deposit referred to in Towfic et al:
...
<BR/>
The column headers have three parts, the BeadChip ID ("4634633002"), the hybridization id ("A", 6 on each BeadChip) and the strip id ("1", two for each hybridization). A sample is flowing freely between the two strips, thus they can be considered as beeing sub-arrays. In the standard preprocessing done with Illumina's Genome Studio, only one probe value is calculated per hybridization, using information from both strips. And this is how the data is formatted in the GEO deposit (GSE40566) originally linked to from Towfic et al. As long as the data from the two strips are handled as technical replicates, the result from analyses using the two different GEO accession, would be quite similar. However, Towfic et al. did not combine the replicates or use a model that handled technical replicates, instead treating these as independent samples. The consequence of inflating the sample size like this is exaggerated confidence, quite similar to the adverse effects of ComBat. 
<BR/>
<BR/>
We chose Towfic et al. as an illustration of adverse effects of ComBat based on the "Methods" description, which did not mention replicate observations. To include the inappropriate replicate handling in our re-analysis would result in hundreds of false positives, even without the use of ComBat. This might confuse the readers, and the example would be less illustrative of the adverse effects of ComBat. Thus, we decided to leave it out in our article, and analyse the data in a more standard way using the GSE40566 as a starting point. However, in this report, we use the GSE61901 deposit in order to replicate exactly some of the tables in Towfic et al.
<br/>
<br/>
**Relationship to ["Gene expression analysis reveals functional pathways of glatiramer acetate activation."](http://informahealthcare.com/doi/abs/10.1517/14728222.2013.778829)**
The GEO deposit that Towfic et al. points to, but did not use (GSE40566), was used by another article by Bakshi et al. In this article they do not count each sample twice and they do not use ComBat, still they find genes that differ between Copaxone and the generic. We have not made any attempt to reproduce the results in Bakshi et al, however based on their description, it seems that the use another batch adjustment tool, the Partek batch remover PARTEKSITERIKNT, might have acted in the same manner as ComBat. In the relevant section of the manual for Partek Genomics Suite v6.6, it is described that the batch effect is estimated with a 2-way Anova and removed from the data, much the same way as ComBat does (excluding the empirical Bayes step). The manual further emphasizes that the resulting data set is ment for visualization purposes and if used in a statistical test, the batch factor must be included. We obtained the manual from Partek user support, but were not allowd to reprint it. From reading the methods section of Bakshi et al. it seems that the data set with batch effects removed was used in the statistical tests without supplying the batch as a factor, which may have lead to false results in a similar way as for Towfic et al.
...
This concludes the exact reproduction. However, due to reasons explained above, we chose to handle the replicate columns as technical replicates in our reproduction and alternative analysis in our article. The rest of this report will show the difference in results from the analyses with or without ComBat, but otherwise conducted in a fairly standard way (in our opinion).
...
Shuffling the N and DP labels within batches produces about the same p-value distribution as for the real data. This is an indication that the significant genes found in the real data are not associated with the N and DP labels, but rather an adverse effect of the ComBat adjustments.
