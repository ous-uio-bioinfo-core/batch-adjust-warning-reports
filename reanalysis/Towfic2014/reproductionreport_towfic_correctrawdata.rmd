...
How the fold change was calculated is not explained. After some trials, the transformation below gave the best correlation with the fold changes listed in Table S5.

```{r}
foldchange_DP_N = meanDP - meanN
foldchange_DP_N = 2^foldchange_DP_N # using non.log values
foldchange_DP_N[foldchange_DP_N<1]=  -(1/foldchange_DP_N[foldchange_DP_N<1])  # downregulation are inverted and made negative.
```
