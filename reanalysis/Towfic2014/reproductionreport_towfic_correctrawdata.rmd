...
includelibs = c("sva", "limma", "xtable", "statmod")
...
sampleannotation = alt_annot
# change some names here so the rest of the script can use the old names.
colnames(sampleannotation)[colnames(sampleannotation)=="Chip"] = "chip"
colnames(sampleannotation)[colnames(sampleannotation)=="ID"] = "covariate"
...
rownames(sampleannotation) = sampleannotation[, "Array Address ID"]
#table(sampleannotation$covariate)
oldsampleannotation = read.table("data/sampleannotation.csv", sep="\t", header=TRUE,  stringsAsFactors=FALSE)
oldsampleannotation$code = make.names(oldsampleannotation$code)
oldsampleannotation$chip = as.character(oldsampleannotation$chip)
dimnames(oldsampleannotation)[[1]] = oldsampleannotation$code
colnames(oldsampleannotation)[colnames(oldsampleannotation)=="covariate"] = "oldcovariate"

### checking difference between old covariate and new.
#comptab = merge(sampleannotation, oldsampleannotation)
#a = comptab$covariate=="DP"
#table(comptab$covariate[a]==comptab$oldcovariate[a])
#a = comptab$covariate=="N"
#table(comptab$covariate[a]==comptab$oldcovariate[a])
#a = comptab$covariate=="M"
#table(comptab$covariate[a]==comptab$oldcovariate[a])
#a = comptab$covariate=="RS"
#table(comptab$covariate[a]==comptab$oldcovariate[a])
#a = comptab$oldcovariate=="RS"
#table(comptab$covariate[a]==comptab$oldcovariate[a])
#comptab[a,]
# RS has 3 less samples (GA.RS.HD)
...
The above GEO accession was not used in Towfic et al. as indicated. Instead the file FORGEO_master_matrix_PLOS_ONE_response.csv(a tab separated text version of the excel file)  was provided by the authors and used .
...
- **FORGEO_master_matrix_PLOS_ONE_response.csv** the measurements in a sample vs probe matrix. Not in github.
...
alt_data=read.table("not_in_github/FORGEO_master_matrix_PLOS_ONE_response.csv", sep="\t", header=TRUE, stringsAsFactors=FALSE, fill=TRUE, skip=5, check.names=FALSE)
...
datamatrix_raw = alt_data

#sort as in the sampleannotation.
#a =match( paste(sampleannotation$chip, sampleannotation$slot, sep="_"),
#			 colnames(Array Address ID))
a = match(sampleannotation[, "Array Address ID"], colnames(datamatrix_raw))
...
Original values from table S2 on X-axis, reproduced values on Y-axis. The values in Table S2 are almost reproduced.
...
How the fold change was calculated is not explained. After some trials and error, it seems that down regulation is inverted and made negative.

In order to correlate them with the reproduced fold changes, we convert them log2 values.

```{r dev='png', fig.width=5, fig.height=5, fig.show='hold', tidy=FALSE}
x = table_s5$Fold_Change
x[x<0] = 1/-x[x<0]
table_s5_log2_foldchange = log2(x)
foldchange_DP_N = meanDP - meanN
plot(table_s5_log2_foldchange, foldchange_DP_N[table_s5$Probe],
     pch=".", xlab="Original Table S5 as log2", ylab="Reproduced",
     main="log2 FC comparison")

```

There seems to be decent consistency between the fold changes in table S5 and the reproduced ones. However, it is the confidence i.e p-values that are affected by the use of ComBat. 
...
The formula "Effect = (GA-generic) - (generic-Medium)" seems strange. And this was a typo. "Effect = (GA-Medium) - (generic-Medium)" was used. This is equivalent to "GA-generic", which we will use.
...
par(mfrow=c(1, 2))
plot(log2(table_s5[,"limma_p"]), log2(limma_p_alt[table_s5$Probe,1]), xlab="Original Table S5", 
...
The reproduced p-values for the Table S5 probes are exactly as the listed ones,

This concludes the reproduction. 

The results can now be compared to the same analysis using an alternative to ComBat.
...
**`r sum(p.adjust(limma_p_woc , method="fdr")<0.05)`**. When ComBat is applied **`r sum(p.adjust(limma_p_alt , method="fdr")<0.05)`** are found.  The p-value distribution is still skewed, but not as much.
...
This sanity check is dropped in this report for the time being due to its long run time. It will be included later.
...
[Table S5](http://www.plosone.org/article/fetchSingleRepresentation.action?uri=info:doi/10.1371/journal.pone.0083757.s013) from Towfic et al. lists the probes with the most average change in expressed between DP(GA) and N(generic) with the results from several test for differential expression. Two of the probes listed and found to be differentially expressed are control probes, (ILMN_1343049 and ILMN_1343048). These are not targeting mouse RNA, but instead target added alien RNA of a known concentration, and are intended to be used in quality control of the hybridization. When these come up in the end of an analysis as differentially expressed genes, something must have gone wrong a long the way. The use of ComBat are one possible explanation since it is able to find differentially expressed genes also for random numbers.
